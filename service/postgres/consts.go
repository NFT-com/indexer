...
var (
	...
	mintEventTableColumns     = []string{"id", "block", "transaction_hash", "collection", "token_id", "owner", "emitted_at"}
	transferEventTableColumns = []string{"id", "block", "transaction_hash", "collection", "token_id", "from_address", "to_address", "emitted_at"}
	saleEventTableColumns     = []string{"id", "block", "transaction_hash", "marketplace", "seller", "buyer", "price", "emitted_at"}
	burnEventTableColumns     = []string{"id", "block", "transaction_hash", "collection", "token_id", "emitted_at"}

	// events on conflict statements
	mintTableOnConflictStatement     = "ON CONFLICT (id) DO UPDATE SET block = EXCLUDED.block, transaction_hash = EXCLUDED.transaction_hash, collection = EXCLUDED.collection, token_id = EXCLUDED.token_id, owner = EXCLUDED.owner, emitted_at = EXCLUDED.emitted_at"
	transferTableOnConflictStatement = "ON CONFLICT (id) DO UPDATE SET block = EXCLUDED.block, transaction_hash = EXCLUDED.transaction_hash, collection = EXCLUDED.collection,token_id = EXCLUDED.token_id, from_address = EXCLUDED.from_address, to_address = EXCLUDED.to_address, emitted_at = EXCLUDED.emitted_at"
	...
	burnTableOnConflictStatement     = "ON CONFLICT (id) DO UPDATE SET block = EXCLUDED.block, transaction_hash = EXCLUDED.transaction_hash, collection = EXCLUDED.collection,token_id = EXCLUDED.token_id, emitted_at = EXCLUDED.emitted_at"
