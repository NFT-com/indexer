...
func NewCreator(log zerolog.Logger, start uint64, template jobs.Parsing, jobs JobRepository, limit uint) *Jobs {

	log = log.With().
		Str("component", "jobs_creator").
		Strs("addresses", template.Addresses).
		Strs("event_types", template.EventTypes).
		Logger()
...
func (c *Jobs) Notify(height uint64) {
	...
	start := uint64(0)
	existing, err := c.jobs.Top("start_height ASC")
	if err != nil && !errors.Is(err, sql.ErrNoRows) {
		c.log.Error().Err(err).Msg("could not check on most recent job")
		return
	}
	if err == nil {
		start = existing.EndHeight + 1
	}
	if start < c.start {
		start = c.start
	}
